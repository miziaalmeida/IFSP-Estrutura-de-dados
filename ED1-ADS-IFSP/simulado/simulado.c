...
void adiciona_nota(Vetor *vet_notas, float nota) {
    // SUA IMPLEMENTAÇÃO
    if((vet_notas->tamanho < vet_notas->capacidade) && (nota >= 0.0 && nota <= 10.0)) {
...
void adiciona_pontos_extras(Vetor *vet_notas, int indice_aluno, float pontos_extras) {
    // SUA IMPLEMENTAÇÃO
    if(indice_aluno < 0) {
...
float nota_media_turma(const Vetor *vet_notas) {
    ...
    float media = soma / vet_notas->tamanho;
    return media;
}
